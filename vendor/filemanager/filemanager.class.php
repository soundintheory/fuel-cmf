...
class Filemanager {
  ...
  public function __construct($config) {
    ...
    $this->public_root = CMFPATH.'modules'.DS.'admin'.DS.'public'.DS.'assets'.DS.'filemanager'.DS;
  ...
  public function getinfo() {
    ...
    $array = array(
			...
			'Preview_Large'=>$this->item['preview_large'],
  ...
  public function preview() {
    ...
    if(isset($this->get['path']) && file_exists($this->doc_root . rawurldecode($this->get['path']))) {
      ...
      header("Content-length: ".filesize($this->doc_root . rawurldecode($this->get['path'])));
      header('Content-Disposition: inline; filename="' . basename(rawurldecode($this->get['path'])) . '"');
      //var_dump(rawurldecode($this->get['path']));exit;
      //$resized_image = rawurldecode('/image/1/64/64'.$this->get['path']);
      //var_dump($resized_image);
      readfile($this->doc_root . rawurldecode($this->get['path']));
      //readfile($rezied_image);
      exit();
    } else {
      $this->error(sprintf($this->lang('FILE_DOES_NOT_EXIST'),rawurldecode($this->get['path'])));
    }
  ...
  private function get_file_info($path='',$return=array()) {
    ...
    if(is_dir($this->doc_root . $path)) {
      ...
      $this->item['preview'] = '/image/1/64/64' . $path; // rawurlencode($path);
       $this->item['preview_large'] = '/image/1/500/0' . $path; // rawurlencode($path);
  ...
  private function loadLanguageFile() {
    ...
    if(file_exists($this->public_root. 'scripts/languages/'.$lang.'.js')) {
      $stream =file_get_contents($this->public_root. 'scripts/languages/'.$lang.'.js');
  ...
  private function availableLanguages() {
    ...
    if ($handle = opendir($this->public_root.'/scripts/languages/')) {
      while (false !== ($file = readdir($handle))) {
        if ($file != "." && $file != "..") {
