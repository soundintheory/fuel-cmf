...
/**
 * MySQL database dump class shamelessly stolen from David Grudl, modified to
 * work correctly for the CMF
 *
	...
	*/
	public function dumpTable($handle, $table)
	{
		...
		while ($row = $res->fetch_assoc()) {
			...
			$numeric[$col] = (bool) preg_match('#^[^(]*(BYTE|COUNTER|SERIAL|INT|CURRENCY|REAL|MONEY|FLOAT|DOUBLE|DECIMAL|NUMERIC|NUMBER)#i', $row['Type']);
		}
		$cols = '(' . implode(', ', $cols) . ')';
		$res->close();
