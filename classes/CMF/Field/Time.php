...
class Time extends Date {
    ...
    public static function process($value, $settings, $model)
    {
        ...
        if (!($value instanceof \DateTime)) $value = \DateTime::createFromFormat(\Arr::get($settings, 'format', 'H:i'), $value);
    ...
    {
        return '<a href="'.$edit_link.'" class="item-link">'.$value->format(\Arr::get($settings, 'list_format', 'H:i')).'</a>';
    }
    ...
    public static function displayForm($value, &$settings, $model)
    {
        ...
        $input = $input = html_tag('div', array( 'class' => 'input-prepend' ), html_tag('span', array( 'class' => 'add-on' ), '<i class="fa fa-clock"></i>').$input);
        
        if (isset($settings['wrap']) && $settings['wrap'] === false) return $label.$input;
        
        return html_tag('div', array( 'class' => 'controls control-group field-type-time'.($has_errors ? ' error' : '') ), $label.$input);
    }
