					...
					children: [ {
						...
						children: [ {
							...
							onShow: function(){
								...
								$.get("/admin/json_link", function(data){
								  generic_pages = JSON.parse(data);
								  var render = '<option value="">Select one ...</option>';
								  for (var i in generic_pages) {
									    render += '<option value="'+generic_pages[i]+'">'+generic_pages[i]+'</option>';
									}
								  $('#'+selectFill.domId + ' select').html(render);
								});
							},
							onChange: function(data){
								var dialog = this.getDialog();
								var generic_page = this.getValue();
								...
								if(generic_page){
									$.get("/admin/json_link/"+generic_page, function(data){ 
									  ...
									  $('#'+elem.domId + ' select').html(render);
									});
									$('#'+elem.domId).css('display','block');
								}else{
									$('#'+elem.domId + ' select').html('');
									$('#'+elem.domId).css('display','none');
								}
							...
							commit: function( data ) {
								...
								data.internal.int_url = this.getValue();
								this.allowOnChange = false;
							}
						}],
						setup: function() {
