(function($) {
	...
	function initItem() {
		...
		function buildNewItem($itemWrap, _data, type) {
			...
			var html = '<span class="img"><img class="gallery-thumb" src="' + CMF.baseUrl + '/image/1/0/90/' + _data['path'] + '" style="width:' + imgWidth + 'px;height:90px;" /></span>' +
		...
		function updateSelected() {
			...
			if (numSelected > 0) {
				$toggleSelection.text(_('admin.common.deselect_all'));
				$selectedActions.show();
			} else {
				$toggleSelection.text(_('admin.common.select_all'));
